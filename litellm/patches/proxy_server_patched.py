...
async def _anthropic_response_to_sse_chunks(response):
    """Convert a completed AnthropicResponse to SSE event dicts for streaming."""
    import uuid as _uuid
    msg_id = getattr(response, "id", f"msg_{_uuid.uuid4().hex[:24]}")
    model = getattr(response, "model", "local")
    ...
    for i, block in enumerate(content_blocks):
        ...
        if btype == "text":
            ...
            yield {"type": "content_block_delta", "index": i, "delta": {"type": "text_delta", "text": btext}}
            yield {"type": "content_block_stop", "index": i}
        elif btype == "tool_use":
            tool_id = getattr(block, "id", f"toolu_{_uuid.uuid4().hex[:24]}")
            tool_name = getattr(block, "name", "")
            tool_input = getattr(block, "input", {})
            import json as _json
            yield {"type": "content_block_start", "index": i, "content_block": {"type": "tool_use", "id": tool_id, "name": tool_name, "input": {}}}
            yield {"type": "content_block_delta", "index": i, "delta": {"type": "input_json_delta", "partial_json": _json.dumps(tool_input)}}
            yield {"type": "content_block_stop", "index": i}
    yield {"type": "message_delta", "delta": {"stop_reason": stop_reason, "stop_sequence": None}, "usage": {"output_tokens": output_tokens}}
...
async def async_data_generator_anthropic(
    ...
    try:
        ...
        if response is None or not hasattr(response, "__aiter__"):
            # ollama_chat/ returns a ModelResponse (non-streaming); convert to SSE pseudo-stream
            async for chunk in _anthropic_response_to_sse_chunks(response):
